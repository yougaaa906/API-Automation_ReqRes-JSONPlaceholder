    ...
    def reqres_login(self) -> str:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                print(f"✅ ReqRes login success | Token: {self.token[:8]}...")
                return self.token
            else:
                print(f"⚠️ ReqRes login failed (status code {resp.status_code}), use mock token")
                self.token = REQRES_MOCK_TOKEN
                return self.token
        except Exception:
            print("⚠️ ReqRes login request exception, use mock token")
            self.token = REQRES_MOCK_TOKEN
            return self.token
